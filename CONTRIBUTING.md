# Contributing to ep_set_title_on_pad

Thanks for helping improve `ep_set_title_on_pad` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_set_title_on_pad` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_set_title_on_pad from your local clone
pnpm run plugins i --path ../ep_set_title_on_pad

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_set_title_on_pad/` are picked up after a server restart.
...
* Etherpad version, ep_set_title_on_pad version, browser + OS
* Steps to reproduce
